...
class TestForm(Form):
    def __init__(self):
        ...
        el = self.fields.add_header('input-els', 'Optional Elements')
        ...
        el = self.fields.add_text('nolabel', defaultval='No Label')
        el.add_note('a note')
        ...
        el = self.fields.add_confirm('confirmeml', 'Confirm Email', match=emel)
        ...
        el = self.fields.add_fixed('fixed-no-label', defaultval = 'fixed no label')
        el = self.fields.add_static('static', 'Static', 'static val')
        el = self.fields.add_static('static-no-label', defaultval='static val no label')

        # want a header for div wrapping only, header element should not actually render
        el = self.fields.add_header('header-for-div-wrap-only')
        el = self.fields.add_text('hfdwo-t1', 'Text1')
        el = self.fields.add_text('hfdwo-t2', 'Text2')
        
        # test header with blank text
        el = self.fields.add_header('header-blank-text', '')
        el = self.fields.add_text('hbt-t1', 'Text1')
        el = self.fields.add_text('hbt-t2', 'Text2')
        
        #test element group with class attribute
        self.fields.add_header('eg-class-attr', 'Element Group with Class Attribute')
        sg = self.fields.add_elgroup('submit-group', class_='submit-only')
