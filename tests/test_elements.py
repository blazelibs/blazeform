...
class CommonTest(unittest.TestCase):
    ...
    def test_conversion(self):
        ...
        form = Form('f')
        el = form.elements.add_text('field', 'Field', 'bool', '1')
        assert el.value is NotGiven
    ...
    def test_type_strings(self):
        ...
        form = Form('f')
        form.elements.add_text('f1', 'Field', 'bool', if_empty='1.25')
        self.assertEqual(form.elements.f1.value, True)
        form.elements.add_text('f2', 'Field', 'boolean', if_empty='1.25')
        self.assertEqual(form.elements.f2.value, True)
        form.elements.add_text('f3', 'Field', 'int', if_empty='1')
        self.assertEqual(form.elements.f3.value, 1)
        form.elements.add_text('f4', 'Field', 'integer', if_empty='1')
        self.assertEqual(form.elements.f4.value, 1)
        form.elements.add_text('f5', 'Field', 'num', if_empty='1.25')
        self.assertEqual(form.elements.f5.value, 1.25)
        form.elements.add_text('f6', 'Field', 'number', if_empty='1.25')
        self.assertEqual(form.elements.f6.value, 1.25)
        form.elements.add_text('f7', 'Field', 'float', if_empty='1.25')
        self.assertEqual(form.elements.f7.value, 1.25)
        form.elements.add_text('f8', 'Field', 'str', if_empty='1.25')
        self.assertEqual(form.elements.f8.value, '1.25')
        form.elements.add_text('f9', 'Field', 'string', if_empty='1.25')
        self.assertEqual(form.elements.f9.value, '1.25')
        form.elements.add_text('f10', 'Field', 'uni', if_empty='1.25')
        self.assertEqual(form.elements.f10.value, u'1.25')
        form.elements.add_text('f11', 'Field', 'unicode', if_empty='1.25')
        self.assertEqual(form.elements.f11.value, u'1.25')
        form.elements.add_text('f12', 'Field', 'bool', if_empty='false')
        self.assertEqual(form.elements.f12.value, False)
        form.elements.add_text('f13', 'Field', 'decimal', if_empty='1.25')
...
class InputElementsTest(unittest.TestCase):
    ...
    def test_el_checkbox(self):
        ...
        el = Form('f').elements.add_checkbox('f', 'f', defaultval='checked')
        self.assertEqual(str(el()), checked)
        ...
        self.assertEqual(str(el()), not_checked)
        el = Form('f').elements.add_checkbox('f', 'f', defaultval=None)
        self.assertEqual(str(el()), not_checked)
        ...
        self.assertEqual(str(el()), checked)
        el = Form('f').elements.add_checkbox('f', 'f', checked='checked')
        self.assertEqual(str(el()), checked)
        ...
        self.assertEqual(str(el()), not_checked)
        el = Form('f').elements.add_checkbox('f', 'f', checked=None)
        self.assertEqual(str(el()), not_checked)
    ...
    def test_el_image(self):
        ...
        el = Form('f').elements.add_image('field', 'Field')
        self.assertEqual(str(el()), html)
    ...
    def test_el_submit(self):
        ...
        html = '<input class="submit" id="f-field" name="submitbtn" type="submit" value="Reset" />'
        form = Form('f')
        el = form.elements.add_submit('field', 'Field', defaultval='Reset', name='submitbtn')
        el.submittedval = '1'
        self.assertEqual( el.value, '1')
        self.assertEqual(str(el()), html)
        ...
        form = Form('f')
        el = form.elements.add_submit('field', 'Field', defaultval='Reset', name='submitbtn', fixed=False)
        el.submittedval = '1'
        self.assertEqual( el.value, '1')
        self.assertEqual(str(el()), html)
    ...
    def test_el_confirm(self):
        try:
            ...
            if 'match argument is required' not in str(e):
                raise
        try:
            Form('f').elements.add_confirm('f', match='notthere')
        ...
        f = Form('f')
        pel = f.elements.add_text('e', 'email')
        cel = f.elements.add_confirm('f', match=pel)
        self.assertEqual(str(cel()), html)
        pel.submittedval = 'foo'
        cel.submittedval = 'foo'
        assert cel.is_valid()
    ...
    def test_el_confirm_invalid(self):
        # empty confirm value
        f = Form('f')
        pel = f.elements.add_password('p', 'password', required=True)
        cel = f.elements.add_confirm('f', match='p')
        pel.submittedval = ''
        cel.submittedval = ''
    ...
    def test_el_email(self):
        html = '<input class="text" id="f-field" name="field" type="text" />'
        el = Form('f').elements.add_email('field', 'Field')
        self.assertEqual(str(el()), html)
...
class SelectTest(unittest.TestCase):
    def test_el_select(self):
        ...
        'b</option>\n</select>'
        el = Form('f').elements.add_select('f', o, defaultval=1, choose=None)
        self.assertEqual(str(el()), html)
        el = Form('f').elements.add_select('f', o, defaultval='1', choose=None)
        self.assertEqual(str(el()), html)
        el = Form('f').elements.add_select('f', o, defaultval=u'1', choose=None)
        self.assertEqual(str(el()), html)
        ...
        assert el.is_valid()
        el = Form('f').elements.add_select('f', o, if_empty=-2, required=True)
        ...
        assert el.is_valid()
        el = Form('f').elements.add_select('f', o, if_empty=1, invalid=['1', '2'])
        assert not el.is_valid()
        ...
        # take precidence.
        el = Form('f').elements.add_select('f', o+[('', 'blank')], if_empty='', required=True)
        assert not el.is_valid()
    ...
    def test_el_select_multi(self):
        ...
        '<option value="1">a</option>\n<option value="2">b</option>\n</select>'
        o = [(1, 'a'), (2, 'b')]
        el = Form('f').elements.add_select('f', o, multiple=True)
        self.assertEqual(str(el()), html)
        el = Form('f').elements.add_select('f', o, multiple=1)
        self.assertEqual(str(el()), html)
        el = Form('f').elements.add_select('f', o, multiple='multiple')
        self.assertEqual(str(el()), html)
        ...
        '<option selected="selected" value="1">a</option>\n<option value="2">'\
        'b</option>\n</select>'
        el = Form('f').elements.add_mselect('f', o, defaultval=1, choose=None)
        self.assertEqual(str(el()), html)
    ...
    def test_el_select_multi1(self):
        ...
        self.assertEqual(el.value, [1])
        el = Form('f').elements.add_mselect('f', o, if_empty=1, auto_validate=False)
        self.assertEqual(el.value, [1])
        el = Form('f').elements.add_mselect('f', o, if_empty=[1,2])
        self.assertEqual(el.value, [1,2])
        el = Form('f').elements.add_mselect('f', o, if_empty=['1','2'])
        self.assertEqual(el.value, ['1', '2'])
        el = Form('f').elements.add_mselect('f', o)
        el.submittedval = ['1','2']
        self.assertEqual(el.value, ['1', '2'])
        ...
        assert el.is_valid()
        el = Form('f').elements.add_mselect('f', o, if_empty=(-1, 1), invalid=['1', '2'])
        assert not el.is_valid()
...
class OtherElementsTest(unittest.TestCase):
    def test_el_textarea(self):
        ...
        el = Form('f').elements.add_textarea('f')
    ...
    def test_el_passthru(self):
        ...
        el = f.elements.add_passthru('f', 'foo')
        assert el.value == 'foo'
        try:
            el.render()
...
class LogicalElementsTest(unittest.TestCase):
    ...
    def test_mcheckbox(self):
        ...
        el = Form('f').elements.add_mcheckbox('f', 'label', group='thegroup', checked=True)
        ...
        self.assertEqual(el(), not_checked)
        el = Form('f').elements.add_mcheckbox('f', 'label', 'foo', 'thegroup', checked=True)
        self.assertEqual(el(), checked)
        el = Form('f').elements.add_mcheckbox('f', 'label', 'foo', 'thegroup')
        self.assertEqual(el(checked='checked'), checked)
        el = Form('f').elements.add_mcheckbox('f', 'label', 'foo', 'thegroup')
    ...
    def test_mcheckbox4(self):
        # test integer values
        f = Form('f')
        el1 = f.elements.add_mcheckbox('f1', 'label', 1, 'thegroup')
        el2 = f.elements.add_mcheckbox('f2', 'label', 2, 'thegroup')
        assert el1.chosen == el2.chosen == False
    ...
    def test_radio(self):
        ...
        el = Form('f').elements.add_radio('f', 'label', group='thegroup', selected=True)
        ...
        self.assertEqual(el(), not_selected)
        el = Form('f').elements.add_radio('f', 'label', 'foo', 'thegroup', selected=True)
        self.assertEqual(el(), selected)
        el = Form('f').elements.add_radio('f', 'label', 'foo', 'thegroup')
        self.assertEqual(el(selected='selected'), selected)
        el = Form('f').elements.add_radio('f', 'label', 'foo', 'thegroup')
    ...
    def test_mradio4(self):
        # test integer values
        f = Form('f')
        el1 = f.elements.add_radio('f1', 'label', 1, 'thegroup')
        el2 = f.elements.add_radio('f2', 'label', 2, 'thegroup')
        assert el1.chosen == el2.chosen == False
        f.set_submitted({'f-submit-flag': 'submitted', 'thegroup':1})
        assert el1.chosen
        assert not el2.chosen
    ...
    def test_dup_values(self):
        ...
        try:
            el = f.elements.add_radio('radio2', 'Radio 2', group='rgroup1' )
...
class LogicalElementsTest2(unittest.TestCase):
    def setUp(self):
        self.f = f = Form('f')
        self.el1 = f.elements.add_mcheckbox('f1', 'label', 1, 'thegroup')
        self.el2 = f.elements.add_mcheckbox('f2', 'label', 2, 'thegroup')
