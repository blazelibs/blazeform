...
class CommonFormUsageTest(unittest.TestCase):
    ...
    def test_first_class_elements(self):
        ...
        form = Form('user')
        form.elements.add_text('username', 'User Name')
        form.elements.add_header('group_membership_header', 'Group Membership')
        form.elements.add_text('groupname', 'Group')
    ...
    def test_is_cancel(self):
        f1 = Form('login')
        f1.elements.add_cancel('cancel', 'Cancel')
    ...
    def test_default(self):
        f = Form('login')
        f.elements.add_text('username', 'User Name')
        f.elements.add_file('file')
    ...
    def test_submit_by_name(self):
        ...
        f = Form('login')
        f.elements.add_text('username', 'User Name', name="unfield")
        f.elements.add_submit('submit', name="submitbtn")
    ...
    def test_blank_checkbox(self):
        ...
        f = Form('login')
        el = f.elements.add_checkbox('disabled', 'Disabled', defaultval=True)
    ...
    def test_blank_checkbox_nameattr(self):
        ...
        f = Form('login')
        el = f.elements.add_checkbox('disabled', 'Disabled', defaultval=True, name="mycb")
        self.assertEqual(el(), html)
        post = {'login-submit-flag': 'submitted'}
        f.set_submitted(post)
    ...
    def test_blank_multiselect(self):
        f = Form('login')
        options = [(1, 'one'), (2, 'two')]
        el = f.elements.add_mselect('numlist', options, 'Disabled', defaultval=2)
    ...
    def test_blank_multicheckbox(self):
        f = Form('login')
        el1 = f.elements.add_mcheckbox('mcheck1', 'Check 1', 1, 'cgroup1', checked=True)
        el2 = f.elements.add_mcheckbox('mcheck2', 'Check 2', 2, 'cgroup1')
    ...
    def test_blank_radio(self):
        f = Form('login')
        el1 = f.elements.add_radio('radio1', 'Radio 1', 1, 'rgroup1', selected=True)
        el2 = f.elements.add_radio('radio2', 'Radio 2', 2, 'rgroup1')
    ...
    def test_is_valid(self):
        ...
        f = Form('f')
        f.elements.add_text('f', required=True)
        # wasn't submitted, so not valid
        assert not f.is_valid()
        f.set_submitted({'f-submit-flag': 'submitted'})
        assert not f.is_valid()
    ...
    def test_form_validators(self):
        ...
        f = Form('f')
        f.elements.add_text('myfield', 'My Field')
    ...
    def test_validator_recursion(self):
        ...
        el = f.elements.add_text('myfield', 'My Field', maxlength=1)
    ...
    def test_validator_element_invalid(self):
        ...
        el = f.elements.add_text('f1', 'f1', maxlength=1)
        el = f.elements.add_text('f2', 'f2')
        f.add_validator(validator)
    ...
    def test_exception_handling(self):
        ...
        el2 = form.elements.add_text('field2', 'Field')
    ...
    def test_submitted_only_when_appropriate(self):
        f1 = Form('login1')
        f1.elements.add_text('field')
        f2 = Form('login2')
        f2.elements.add_text('field')
